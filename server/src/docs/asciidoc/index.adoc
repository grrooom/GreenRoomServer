...
[cols="5,6,10,6", options="header"]
|===
| Http Status | Http Status Code | 비고 | Response Body
| OK | 200 | 요청에 성공  | O
| CREATED | 201 | resource 생성 | O
| NO CONTENT | 204 | 요청에 성공했으나 반환할 응답값이 없음 | X
|===
...
이 문서에 명시된 실패 응답 외에 서버 측에서 처리하지 못한 예외가 발생하면 다음 error가 공통적으로 반환됩니다.
...
Authorization header가 요구되는 모든 API 요청에서 발생 할 수 있는 ERROR 응답입니다.

==== 1. Access Token을 header에서 찾을 수 없음

include::{snippets}/jwt/error1/http-response.adoc[]

==== 2. 부적절한 Access token이 전달됨
...
*별도의 Authorization Header가 필요하지 않습니다.*
...
==== Response Fields
include::{snippets}/api/users/logout/1/response-fields.adoc[]
...
==== Response Fields
include::{snippets}/api/users/delete/1/response-fields.adoc[]
