...
public enum ResponseCodeEnum {
    ...
    //200~
    SUCCESS(HttpStatus.OK,"A001","요청에 대한 처리 성공",""),
    ...
    INVALID_REQUEST_PARAMETER(HttpStatus.BAD_REQUEST,"C013","서버 내부 문제로 부적절한 request parameter가 전달됨.","서버에 문의"),
    INVALID_IMAGE_FORMAT(HttpStatus.BAD_REQUEST,"C014","올바르지 못한 image format이 전달됨.","적절한 image format으로 다시 전송"),
    OAUTH_USER_ALREADY_EXIST_CONNECT_AVAILABLE(HttpStatus.CONFLICT,"C015","같은 이름의 user가 oauth2에 이미 존재. 연동 가능함.","사용자에게 연동 여부 질의 : 동의->계정 연동 api 요청 & 미동의->oauth 로그인 제한"),
    PASSWORD_NOT_MATCHED(HttpStatus.CONFLICT, "C016","로그인 password가 일치하지 않음.","password 다시 전송"),
    VERIFIED_USER_ALREADY_EXISTS(HttpStatus.CONFLICT,"C017","이미 해당 email로 가입된 회원이 존재함.","해당 email 회원가입 제한"),
    EXCEED_NUMBER_OF_TRIAL_VERIFICATION(HttpStatus.CONFLICT,"C018","인증 시도 횟수를 초과함","마지막 요청으로부터 15분 후 다시 시도 가능"),
    EMAIL_VERIFICATION_CODE_EXPIRED(HttpStatus.CONFLICT,"C019","email 인증 코드가 만료됨.","email 인증 다시 요청 필요"),
    EMAIL_VERIFICATION_CODE_NOT_MATCHED(HttpStatus.CONFLICT,"C020","email 인증 코드가 일치하지 않음.",""),
    EMAIL_NOT_VERIFIED(HttpStatus.CONFLICT,"C021","인증되지 않은 email로 회원가입을 시도함","회원가입 제한 & email 인증 요구"),
    INVALID_EMAIL_CONTENT(HttpStatus.CONFLICT,"C022","email을 보낼 수 없는 부적절한 contents가 전달됨 :부적절한 email 또는 email 본문","전달된 딥링크가 적절한지 확인 후 다시 요청"),
    ALREADY_VERIFIED_EMAIL(HttpStatus.CONFLICT,"C0223","이미 인증된 email로 이메일 인증을 시도함.",""),
    REFRESH_TOKEN_NOT_EXISTS(HttpStatus.NOT_FOUND,"C024","사용자의 refresh token이 존재하지 않아 갱신이 불가능함.","다시 로그인 필요"),
    INVALID_REQUEST_ARGUMENT(HttpStatus.BAD_REQUEST,"C025","요청시 request body로 전달된 argument가 조건을 만족시키지 않음. ","명세된 request 조건에 맞는 요청 다시 전송 필요"),
